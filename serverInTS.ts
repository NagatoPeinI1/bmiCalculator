...
app.post("/postBGMI", (req, res) => {
    ...
    clients.forEach((client) => {
      let ishealthy: string, bmiCategory: string, bmiValue: number;
      bmiValue = (10000 * client.WeightKg) / (client.HeightCm * client.HeightCm);
      if (bmiValue <= 18.4) {
    ...
    const listofOverweightClients: JSON[] = finalData.filter((weightData: any) => {
      if ([
          "Overweight",
          "Moderately obese",
          "Severely obese",
